...
const MENU_CATEGORIES = [
  ...
  { value: 'tandoor', label: 'Tandoor Specials' },
...
const DEFAULT_MENU = [
  { id: 1, name: "Samosa", category: "starters", price: 6.99, description: "Crispy pastry filled with spiced potatoes and peas", image: "https://images.unsplash.com/photo-1601050690597-df0568f70950?w=600&q=80" },
  { id: 2, name: "Onion Bhaji", category: "starters", price: 7.99, description: "Crispy fried onion fritters with mint chutney", image: "https://images.unsplash.com/photo-1606491956689-2ea866880049?w=600&q=80" },
  { id: 3, name: "Butter Chicken", category: "curries", price: 17.99, description: "Tender chicken in rich tomato-cream sauce with butter", image: "https://images.unsplash.com/photo-1603894584373-5ac82b2ae398?w=600&q=80" },
  { id: 4, name: "Lamb Rogan Josh", category: "curries", price: 19.99, description: "Slow-cooked lamb in aromatic Kashmiri spices", image: "https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=600&q=80" },
  { id: 5, name: "Palak Paneer", category: "curries", price: 14.99, description: "Creamed spinach with cubes of Indian cottage cheese", image: "https://images.unsplash.com/photo-1609183480237-ccf98826d18c?w=600&q=80" },
  { id: 6, name: "Tandoori Chicken", category: "tandoor", price: 16.99, description: "Yogurt-marinated chicken roasted in clay oven", image: "https://images.unsplash.com/photo-1610057099570-7a4e8d9d5a8b?w=600&q=80" },
  { id: 7, name: "Garlic Naan", category: "breads", price: 4.99, description: "Soft bread with garlic and butter from tandoor", image: "https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=600&q=80" },
  { id: 8, name: "Biryani", category: "breads", price: 16.99, description: "Fragrant basmati rice with saffron and your choice of protein", image: "https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?w=600&q=80" },
  { id: 9, name: "Gulab Jamun", category: "desserts", price: 7.99, description: "Warm milk dumplings in rose-flavored syrup", image: "https://images.unsplash.com/photo-1666190070082-db05ac84cd2e?w=600&q=80" },
  { id: 10, name: "Mango Lassi", category: "drinks", price: 5.99, description: "Creamy yogurt drink blended with fresh mango", image: "https://images.unsplash.com/photo-1527661591475-527312dd65f5?w=600&q=80" }
];

function getMenu() {
  let stored = localStorage.getItem('restaurant_menu');
...
function editMenuItem(id) {
  let item = getMenu().find(i => i.id === id);
  if (!item) return;
  ...
  document.getElementById('menuSubmitBtn').innerHTML = '<i class="fas fa-save me-2"></i> Save Changes';
...
function initMenuForm() {
  let categorySelect = document.getElementById('menuCategory');
  if (categorySelect) {
    categorySelect.innerHTML = '<option value="">Select category</option>' +
      MENU_CATEGORIES.map(c => `<option value="${c.value}">${c.label}</option>`).join('');
  }
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (editId) {
      let idx = menu.findIndex(i => i.id === parseInt(editId));
      if (idx !== -1) {
        menu[idx] = { ...menu[idx], ...itemData };
      }
    } else {
      let maxId = menu.reduce((max, i) => Math.max(max, i.id), 0);
      menu.push({ id: maxId + 1, ...itemData });
    }
    saveMenu(menu);
    renderMenuTable();
    cancelEdit();
...
function renderCharts() {
  if (!isAdminPage()) return;
  let orders = getOrders();
  let primaryColor = '#C41E3A';
  let secondaryColor = '#FF6F00';
...
function seedDemoData() {
  ...
  let demoOrders = [];
  let names = ['Raj Patel', 'Priya Sharma', 'Michael Chen', 'Sarah Johnson', 'Amit Kumar'];
  let menuItems = [
    { name: 'Samosa', price: 6 },
    { name: 'Butter Chicken', price: 17 },
    { name: 'Lamb Rogan Josh', price: 19 },
    { name: 'Palak Paneer', price: 14 },
    { name: 'Tandoori Chicken', price: 16 },
    { name: 'Biryani', price: 16 },
    { name: 'Garlic Naan', price: 4 },
    { name: 'Gulab Jamun', price: 7 }
  ];
  for (let i = 0; i < 15; i++) {
    ...
    demoOrders.push({
      ...
      address: Math.floor(100 + Math.random() * 900) + ' Curry Lane',
      time: d.toISOString()
    });
  }
