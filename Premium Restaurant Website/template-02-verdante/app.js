/* VERDANTE — Modern Botanical Kitchen
...
/* ---------------- Navbar scroll state ---------------- */
const verNav = document.getElementById('verNav');
function navScroll() {
  if (!verNav) return;
  if (window.scrollY > 60) verNav.classList.add('scrolled');
  else verNav.classList.remove('scrolled');
}
window.addEventListener('scroll', navScroll);
navScroll();

/* Close mobile menu on link click */
document.querySelectorAll('#verNavbar .nav-link').forEach(link => {
  link.addEventListener('click', () => {
    const col = document.getElementById('verNavbar');
...
/* ---------------- Reveal on scroll ---------------- */
const revealEls = document.querySelectorAll('.reveal, .reveal-left, .reveal-right, .reveal-scale, .reveal-up, .reveal-tilt');
...
function showToast(message, icon) {
  ...
  t.className = 'toast show toast-lux';
  t.style.cssText = 'display:flex;align-items:center;gap:10px;padding:14px 20px;min-width:260px;box-shadow:0 14px 40px rgba(14,36,28,.22);animation:fadeSlideIn .4s var(--ease);';
...
==================================================== */
function getCart() {
  try { return JSON.parse(localStorage.getItem('verdante_cart') || '[]'); } catch (e) { return []; }
}
function saveCart(cart) {
  localStorage.setItem('verdante_cart', JSON.stringify(cart));
  updateCartCount();
}
...
function clearCart() {
  localStorage.removeItem('verdante_cart');
  updateCartCount();
  renderCartPage();
}
...
function initCheckout() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    orders.push(order);
    localStorage.setItem('verdante_orders', JSON.stringify(orders));
...
/* ---------------- Orders (shared) ---------------- */
function getOrders() {
  try { return JSON.parse(localStorage.getItem('verdante_orders') || '[]'); } catch (e) { return []; }
}
function saveOrders(orders) { localStorage.setItem('verdante_orders', JSON.stringify(orders)); }

/* ====================================================
   MENU DATA
==================================================== */
const MENU = [
  { id: 1, name: 'Burrata & Nectarine', category: 'starters', price: 26, description: 'Creamy burrata, charred nectarine, basil oil, aged balsamic', image: 'https://images.unsplash.com/photo-1541516160071-4bb0c5af65ba?w=600&q=80', tag: 'Signature' },
  { id: 2, name: 'Golden Beet Carpaccio', category: 'starters', price: 22, description: 'Shaved golden beet, whipped goat cheese, candied walnuts, sorrel', image: 'https://images.unsplash.com/photo-1540189549336-e6e99c3679fe?w=600&q=80', tag: 'New' },
  { id: 3, name: 'Charred Octopus', category: 'starters', price: 34, description: 'Slow-poached octopus, smoked paprika, potato espuma, parsley', image: 'https://images.unsplash.com/photo-1476224203421-9ac39bcb3327?w=600&q=80' },
  { id: 4, name: 'Crispy Polenta & Mushrooms', category: 'starters', price: 24, description: 'Crispy polenta, wild mushrooms, truffle cream, chives', image: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600&q=80', tag: 'Vegetarian' },
  { id: 5, name: 'Herb-Roasted Lamb Shoulder', category: 'mains', price: 66, description: 'Twelve-hour lamb, charred leeks, rosemary jus, garden mint', image: 'https://images.unsplash.com/photo-1544025162-d76694265947?w=600&q=80', tag: 'Signature' },
  { id: 6, name: 'Sea Bass al Forno', category: 'mains', price: 54, description: 'Whole sea bass, fennel, citrus, green olive salsa', image: 'https://images.unsplash.com/photo-1519708227418-c8fd9a32b7a2?w=600&q=80' },
  { id: 7, name: 'Tagliatelle al Tartufo', category: 'mains', price: 42, description: 'Hand-cut tagliatelle, black truffle, parmesan emulsion', image: 'https://images.unsplash.com/photo-1473093295043-cdd812d0e601?w=600&q=80', tag: 'New' },
  { id: 8, name: 'Smoked Pork Belly', category: 'mains', price: 48, description: 'Slow-smoked belly, apple purée, cavolo nero, crackling', image: 'https://images.unsplash.com/photo-1546833999-b9f581a1996d?w=600&q=80' },
  { id: 9, name: 'Garden Risotto Verde', category: 'vegetarian', price: 36, description: 'Green pea risotto, mint, ricotta salata, lemon zest', image: 'https://images.unsplash.com/photo-1476124369491-e7addf5db371?w=600&q=80', tag: 'Vegan option' },
  { id: 10, name: 'Heirloom Tomato Pasta', category: 'vegetarian', price: 32, description: 'Strozzapreti, heirloom tomato, basil, burrata cream', image: 'https://images.unsplash.com/photo-1551183053-bf91a1d81141?w=600&q=80' },
  { id: 11, name: 'Charred Cauliflower Steak', category: 'vegetarian', price: 30, description: 'Charred cauliflower, tahini, pomegranate, smoked almonds', image: 'https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=600&q=80' },
  { id: 12, name: 'Lemon Olive Oil Cake', category: 'desserts', price: 18, description: 'Olive oil sponge, lemon curd, mascarpone, summer berries', image: 'https://images.unsplash.com/photo-1565958011703-44f9829ba187?w=600&q=80', tag: 'Signature' },
  { id: 13, name: 'Panna Cotta & Rhubarb', category: 'desserts', price: 16, description: 'Vanilla panna cotta, poached rhubarb, pistachio crumble', image: 'https://images.unsplash.com/photo-1488477181946-6428a0291777?w=600&q=80' },
  { id: 14, name: 'Artisan Cheese Board', category: 'desserts', price: 27, description: 'Local cheeses, honeycomb, fig jam, rosemary crisp', image: 'https://images.unsplash.com/photo-1452195100486-9cc805987862?w=600&q=80' },
  { id: 15, name: 'Sage & Grapefruit Spritz', category: 'drinks', price: 18, description: 'House aperitif, sage, grapefruit, sparkling wine', image: 'https://images.unsplash.com/photo-1536935338788-846bb9981813?w=600&q=80', tag: 'Mixologist' },
  { id: 16, name: 'Barbera d\'Asti', category: 'drinks', price: 58, description: 'Red Piedmont — bright cherry, silky finish', image: 'https://images.unsplash.com/photo-1553361371-9b22f78e8b1d?w=600&q=80' }
];

function getMenu() {
  try { const s = JSON.parse(localStorage.getItem('verdante_menu')); return Array.isArray(s) && s.length ? s : MENU; } catch (e) { return MENU; }
}
function saveMenu(menu) { localStorage.setItem('verdante_menu', JSON.stringify(menu)); }
...
function initReservation() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    const list = JSON.parse(localStorage.getItem('verdante_reservations') || '[]');
    list.push(res);
    localStorage.setItem('verdante_reservations', JSON.stringify(list));
...
function initContact() {
  ...
  form.addEventListener('submit', function (e) {
    ...
    const list = JSON.parse(localStorage.getItem('verdante_messages') || '[]');
    list.push(msg);
    localStorage.setItem('verdante_messages', JSON.stringify(list));
        ...
        <div class="order-card">
          ...
          <div class="oc-body">
            ...
            <div class="d-flex justify-content-between align-items-center mt-2">
              ...
              <button class="btn-emerald oc-btn" onclick="addToCart(${item.id})"><i class="fas fa-plus"></i> Add</button>
            </div>
          </div>
        </div>
      </div>
...
function deleteMenuItem(id) {
  ...
  localStorage.setItem('verdante_menu', JSON.stringify(menu));
  renderMenuManagement();
...
function renderCharts() {
  const orders = getOrders();
  const emerald = '#147A5B';
  const emeraldSoft = 'rgba(20,122,91,.15)';
  const emeraldLine = 'rgba(20,122,91,.08)';
  ...
  if (salesCtx && window.Chart) {
    new Chart(salesCtx, {
      type: 'bar',
      data: { labels: weekLabels, datasets: [{ label: 'Sales ($)', data: weekData, backgroundColor: emerald, borderRadius: 6, maxBarThickness: 40 }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: emeraldLine }, ticks: { color: '#6E7A74', callback: v => '$' + v } }, x: { grid: { display: false }, ticks: { color: '#6E7A74' } } } }
    });
  }
  const dayLabels = [];
  const dayData = [];
  ...
  if (dailyCtx && window.Chart) {
    new Chart(dailyCtx, {
      type: 'line',
      data: { labels: dayLabels, datasets: [{ label: 'Sales ($)', data: dayData, borderColor: emerald, backgroundColor: emeraldSoft, fill: true, tension: 0.45, pointRadius: 4, pointBackgroundColor: emerald }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: emeraldLine }, ticks: { color: '#6E7A74', callback: v => '$' + v } }, x: { grid: { display: false }, ticks: { color: '#6E7A74' } } } }
    });
  }
}
...
function seedDemoData() {
  if (localStorage.getItem('verdante_orders')) return;
  ...
  for (let i = 0; i < 18; i++) {
    ...
    orders.push({
      ...
      phone: '(415) 555-' + Math.floor(1000 + Math.random() * 9000),
      email: 'guest@example.com',
      address: Math.floor(100 + Math.random() * 900) + ' Dolores Street, San Francisco',
      time: d.toISOString()
    });
  }
  localStorage.setItem('verdante_orders', JSON.stringify(orders));
  ...
  const res = [
    { id: 'RES-1001', name: 'Isabella Whitmore', email: 'isabella@example.com', phone: '(415) 555-0111', guests: '2', date: '2026-08-08', time: '19:30', occasion: 'Anniversary', notes: '', status: 'confirmed', created: new Date().toISOString() },
    { id: 'RES-1002', name: 'Marcus Bennett', email: 'marcus@example.com', phone: '(415) 555-0222', guests: '4', date: '2026-08-10', time: '20:00', occasion: 'Business', notes: '', status: 'pending', created: new Date().toISOString() },
    { id: 'RES-1003', name: 'Victoria Laurent', email: 'victoria@example.com', phone: '(415) 555-0333', guests: '6+', date: '2026-08-12', time: '18:30', occasion: 'Private Event', notes: '', status: 'pending', created: new Date().toISOString() }
  ];
  localStorage.setItem('verdante_reservations', JSON.stringify(res));
}
...
function openLightbox(src, alt, caption) {
  ...
  if (!lb) {
    ...
    lb.id = 'lightbox';
    lb.style.cssText = 'position:fixed;inset:0;z-index:5000;background:rgba(10,24,19,.95);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;opacity:0;visibility:hidden;transition:all .4s var(--ease);';
    lb.innerHTML = `<button style="position:absolute;top:24px;right:30px;background:none;border:none;color:var(--emerald-light);font-size:2rem;cursor:pointer;"><i class="fas fa-times"></i></button>
      <img style="max-width:88vw;max-height:72vh;object-fit:contain;border:1px solid rgba(63,174,133,.35);box-shadow:0 30px 80px rgba(0,0,0,.6);"/>
